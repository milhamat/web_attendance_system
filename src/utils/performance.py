...
def embed_convert(vector_embed):
        conv = list(map(float, vector_embed.strip("[]").split(",")))
        return np.array(conv, dtype=np.float32) 

def count_similarity(query_vector: np.ndarray, current_vector: np.ndarray, thres:float) -> float:
...
# SIMIILAR FACE
for a, b in pairs:
    f1 =  embed_convert(face1[a][1])
    f2 =  embed_convert(face1[b][1])
    pred.append(count_similarity(f1, f2, THRESHOLD))  
    
# DISIMILAR FACE
for n in range(10):
    f1 =  embed_convert(face1[random.randint(0, 4)][1])
    f2 =  embed_convert(face2[random.randint(0, 4)][1])
    pred.append(count_similarity(f1, f2, THRESHOLD))
